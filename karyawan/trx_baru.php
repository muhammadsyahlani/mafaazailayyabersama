...
$sql = "SELECT tmp_trx.*, barangjasa.* FROM tmp_trx, barangjasa WHERE tmp_trx.id_brg=barangjasa.id_brg 
		AND tmp_trx.id_karyawan='$karyawan' AND tmp_trx.status='On Process'";
$query = mysqli_query($conn,$sql);
	...
	$sqltmp = "UPDATE tmp_trx SET
			id_trx='". $no ."',
			status='". $stt ."'
			WHERE id_karyawan='". $karyawan ."' AND status='". $stts ."'";
	$resstmp = mysqli_query($conn, $sqltmp);	

	$sqltrx="INSERT INTO trx(id_trx,id_kon,tgl_trx,total,id_karyawan)
		  VALUES('$no','$kon','$tg','$grand','$karyawan')";
								...
								<table class="table table-striped table-bordered table-hover" id="tabel-data">
									...
									<tbody>
										<?php
											$i = 1;
											$grand=0;
											$sql = "SELECT tmp_trx.*, barangjasa.*, karyawan.* FROM tmp_trx, barangjasa, karyawan WHERE
													tmp_trx.id_brg=barangjasa.id_brg AND tmp_trx.id_karyawan=karyawan.id_karyawan
													AND tmp_trx.status='On Process' AND tmp_trx.id_karyawan='$karyawan' ORDER BY barangjasa.nama ASC";
